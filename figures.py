...
def car(x, y, k=1):
    ...
    equal_trapeze(x + 2 * k, y + 2 * k, 9 * k, 5 * k, 2 * k, color='Cream')
...
def home(x, y, k=1):
    ...
    triangle_90(x + 2 * k, 7 * k, k * m.sqrt(18), color='Green', angle_round=135)


def wheelchair(x, y, k=1):
    rectangle(x, y, 8 * k, 2 * k, color='Pink')
    square(x + 2 * k, y + 2 * k, 2 * k, color='Lilac')
    triangle_90(x + 2 * k, y + 2 * k, 2 * k, 2 * k, color='Lilac', angle_round=270)
